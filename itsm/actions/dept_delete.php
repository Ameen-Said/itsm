...
$auth->requireLogin();
if (!$auth->verifyCsrfToken($_GET['csrf_token']??null)) { flash('danger','CSRF error.'); redirect(APP_URL.'/pages/departments.php'); }
$auth->requirePermission('departments','delete');
$id=(int)($_GET['id']??0);
if ($id) {
    try {
        $db->prepare("DELETE FROM `departments` WHERE id=?")->execute([$id]);
        $auth->logAudit($auth->getUserId(),'delete','departments',$id);
